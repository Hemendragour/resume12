          ...
          return (
            <div key={index}>
              <div className="flex items-start justify-between gap-4">
                <h3 className="text-[12px] font-bold text-rose-700">{project.title}</h3>
                <div className="flex flex-col items-end shrink-0">
                  ...
                  {(project.link || project.github) && (
                    <div className="mt-1 flex gap-3 text-[11px]">
                      {project.link && (
                        <a href={project.link} target="_blank" rel="noreferrer" className="text-rose-600 hover:underline">
                          Live Demo
                        </a>
                      )}
                      {project.github && (
                        <a href={project.github} target="_blank" rel="noreferrer" className="text-rose-600 hover:underline">
                          GitHub
                        </a>
                      )}
                    </div>
              ...
              {project.technologies?.length > 0 && (
                <p className="mt-1 text-[11px] text-slate-500">{project.technologies.join(" • ")}</p>
              )}
              {bullets.length > 0 && (
